...
**Access state:** **GRANTED 2026-09-19 (owner).** Key placed in repo `.env` 2026-09-21. **EXP-JEV-1 executed 2026-09-21 with first-party numbers: KILL SIGNAL** — Jev 0.208 accuracy / 0.201 Brier vs deterministic baseline 0.542 / 0.142 on the 24-case scoreable set (close class collapsed to 1/13 into "contain"; no FP-confidence inflation). Report: `docs/research/jev-expjev1-report-2026-09-21.md`. **Model-pin amendment (2026-09-21, live API):** request ids `jev-1.13` and `jev-1.13.x` return 400 "Unknown model"; the EXACT version `jev-1.13.0` is accepted (200) — the original "pin `jev-1.13.x`" instruction is satisfied by requesting `jev-1.13.0` directly (`/v1/models` lists only `jev-latest`/`jev-preview`, release_date 2026-09-10, and `jev-latest` serves `jev-1.13.0`). Key location: single live copy in `~/Projects/.env.local` (shared across projects; documented in `~/Projects/.env.example` + `runbooks/jev-typesafe.md`). Live request schema recorded in `tools/jev-replay/jev-client.mjs` and `tools/README.md`. An internal backlog-triage pass (`tools/jev-prioritize/`, 101 open items) ran 2026-09-21 under this kill-signal caveat: `docs/agent-artifacts/jev-priority-pass-2026-09-21.md`. §1–2 general claims remain sourced-only; first-party evidence now exists for the triage corpus (negative) and an uncalibrated priority pass.
...
**Feasibility amendment (2026-09-19, data inventory):** the constructible corpus is ~28 cases of which ~24 are scoreable; the independent, outcome-labeled class is SEC-001..008 + RT-001..004 + Mimosa 12/13 (narrative-reconstructed from the epistemic audit §15; the raw scanner text is not recoverable and the 13th finding is unaccounted). Fixture-derived labels are circular vs. the deterministic baseline (same rules authored both) and are excluded from scoring. No native "escalate"-class exemplar exists — the close/escalate/contain mapping is experimenter-imposed pending owner ratification. Consequence, recorded in the JEV register: **EXP-JEV-1 v1 is a calibration smoke test with a kill signal (materially-worse-than-baseline, or confidence inflation on the known FP class); per-class calibration-error claims require the corpus expansion steps** (raw Mimosa text recovery, synthetic FP/stale generation, owner-ratified label mapping).
...
|---|---|---|---|
| **EXP-JEV-1** | Calibration replay of security-finding history (fixture corpora + Mimosa episode) through Jev on triage judgments (close/escalate/contain) | Calibration report: per-class accuracy, calibration error vs known outcomes, cost/case. **Kill criterion:** calibration materially worse than regex/heuristic baseline on the same corpus, or inflated confidence on the known false-positive class | API access (NM-R15). **EXECUTED 2026-09-21: KILL SIGNAL fired** (0.208/0.201 vs baseline 0.542/0.142; escalation class matched baseline 4/11, close class collapsed 1/13, FP conf deflated 0.407). Owner kill-or-continue call is now the live item; corpus-expansion steps only if continuing. |
...
| **EXP-JEV-4** | Prompt-injection red-team on `state`: adversarial PDF-text probes (instructions smuggled in document content) against all three primitives; measure refusal/robustness | Threat-model addendum for the application-security audit; **hard gate** before any customer-facing consideration | API access. **EXECUTED 2026-09-21** (`jev-expjev4-injection-report-2026-09-21.md`): zero genuine steering across 24 adversarial primitive-judgments; framing free; Jev mode-collapses on long document-text states — falsifies Jev-on-extracted-document-text on attack AND utility grounds. Full clearance suite deferred behind the JEV-1 kill-or-continue call. |
...
| 2026-09-19 | **Access granted.** Replay harness built: `tools/jev-replay/{jev-client,corpus,baseline,replay}.mjs` (model pinned `jev-1.13`; raw captures gitignored; baseline validated — 0.542 accuracy / 0.142 Brier / 0.737 mean-FP-confidence on 24 scoreable cases). Corpus feasibility amendment added above §4 (smoke-test scope, no native escalate exemplar, circular fixture labels excluded). Awaiting: key in `.env`, owner ratification of the label mapping (JEV register, task-inventory). |
| 2026-09-21 | **EXP-JEV-1 executed — KILL SIGNAL** (`jev-expjev1-report-2026-09-21.md`). Key placed in `.env`. Live-API corrections: request ids `jev-1.13`/`jev-1.13.x` are invalid (400); exact `jev-1.13.0` is valid and is what the client now pins (`jev-latest` also serves `jev-1.13.0`). Key single-sourced in `~/Projects/.env.local`. Request schema (dict-of-tagged-variants; score criteria = level-label list; choice criteria = option→description dict) live-validated and encoded in `jev-client.mjs`. Internal backlog priority pass over 101 open items run under the kill-signal caveat: `docs/agent-artifacts/jev-priority-pass-2026-09-21.md` + `tools/jev-prioritize/`. Owner call now open: kill the program per the pre-registered criterion, or fund corpus expansion (raw Mimosa text recovery, synthetic FP/stale generation) + label-mapping ratification and re-run. |
| 2026-09-21 | **EXP-JEV-4 executed — injection red-team** (`jev-expjev4-injection-report-2026-09-21.md`, runner `tools/jev-replay/injection.mjs`, noul schema validated: `instructions` string). 8 adversarial styles × 3 primitives × raw/framed, paired benign controls. Zero genuine steering (choice obedience 0/8 after mode-collapse correction; score max-injections 0/6; noul 0/8); base64-obfuscated directives produced drift without steering; untrusted-data framing cost nothing. Load-bearing corollary: Jev mode-collapses on long document-text states (do-later 15/16, scores 3–4.8 regardless of content) — **Jev-on-extracted-document-text is falsified on attack and utility grounds together** (J-04 whole-page shape dead; short structured states unaffected). Customer-facing hard gate retained; clearance suite deferred behind the JEV-1 owner call. |
