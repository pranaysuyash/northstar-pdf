...
| RG-014 | Signed-document behavior | Native/web/security | `PARTIAL` | Detection + invalidation guard delivered 2026-08-25 (`web/pdf-signature-guard.mjs`): detects AcroForm `/SigFields`, `/SigFlags`, and standalone signature dictionaries (`/ByteRange`+`/Contents`) via pikepdf; edit sessions blocked unless `signatureAcknowledged=true` (explicit human acknowledgment); malformed detection facts rejected (falsifier). Validated on the real unsigned fixture and a synthetic signed-structure fixture. Cryptographic signature validity verification and real signed-document corpus remain open |
