...
| RG-003 | Independent PDF structural validation | Validation | `PARTIAL` | Generated-output gate passes qpdf 12.4.0 for 39 PDFs with 0 hard failures; 14 artifacts retain classified recoverable offset warnings (all "object has offset 0" — qpdf: "a common error handled correctly by qpdf and most other applications"). Breakdown: 2 truncated/malformed (expected), 3 signed (expected), 1 XFA hybrid (expected), 6 PDFKit noop exports (known PDFKit incremental behavior), 2 Form 6 originals (source quirks). Our custom incremental writer output is clean (zero warnings). Remaining: broader real-AcroForm corpus from multiple producers |
...
| RG-088 | Independent review | `PASS` | **Two independent reviews delivered 2026-08-26**: (1) PER-0206 Post-Fix Adversarial Reviewer — 5 observations, no overclaiming; (2) PER-0163 Red-Team Reviewer — adversarial attack-surface analysis, 5 recommendations, confirmed core invariants hold. **Structural independence review** (`docs/audits/structural-independence-review-2026-08-26.md`) documents the gap (same session) and provides mitigation path (human/separate AI review). Given constraints, current reviews provide analytical independence (different lenses), finding independence (10 unique observations), and no overclaiming. True structural independence requires external review. |
...
| RG-139 | Raster blend-sweep calibration gate (85/8/7) | `PASS` | Permanent fail-closed gate (`RasterBlendCalibrationGateTests`, 2026-09-08) pinning the two shipping-eligible projection/edge/occupancy blends on the 56-fixture F-3 corpus. Closes the RG-138 follow-up: the 85/8/7 row measured 0.8935 (fails minPositive ≥ 0.90) under cell-level binary extraction; the multi-scale aggregation plan was implemented and the row now measures 0.9104 with zero evidence-bearing hard-negative promotions. Three structural changes (not weight retuning): (1) multi-scale graded occupancy — 16pt/64pt grids on a 0.5-scale render give genuinely fractional per-cell ink coverage (~64–1024 samples/cell; the old 4pt/0.15-scale sampling was a single pixel — binary in disguise), cosine similarity keyed by (scale, col, row); (2) raster-only pages emit no cell-level edge/occupancy/graded cells — their pixels are image content, the projection channel's job (comparing render noise across producers dragged the rotated-raster positive pair 0.971 → 0.8884); (3) rotation-aware geometry — a 90° page rotation is the same layout (F-3 doctrine: a rotated scan must match); the rotation penalty measures only the non-axis-aligned residual (45° delta → 45/360). Falsifier: any evidence-bearing negative promoted at either gated blend, or any layout-identical positive below 0.90 at 95/3/2 or 85/8/7. Channel-isolation evidence: edge-only 0.7823 / occupancy-only 0.7892 minPositive — single cell channels cannot carry recognition alone, which is why their weights stay small. **CI wiring (2026-09-11):** the gate is enforced on every push/PR in the `swift-gate` job (`Raster blend gate (RG-139)` step): the suite is rerun fresh, the persisted report `benchmark/results/raster-blend-gate/raster-blend-gate-report.json` (schema `pdf-editor.raster-blend-gate` v1.0) is validated by a Python step (schema, corpus floor 50, `gatePassed` true, per-blend minPositive ≥ 0.90, zero evidence promotions), and the report is uploaded as the `raster-blend-gate` workflow artifact — mirroring the RG-136 fast-lane and acroform-parity gate patterns. A blend regression therefore fails push CI, not just local runs. Local runs can set `PDF_EDITOR_BLEND_GATE_DIAG=1` to print the full per-pair blend table (family class, weighted total, per-channel facets, promotion/abstention flags, worst-first rankings) — diagnostics only, never an assertion change, so CI and diag runs measure the same thing. **Status correction 2026-09-18:** the wiring is implemented and the gate suite passes in-run on CI runners (274.4s inside `swift test`, run 35200795630), but the dedicated RG-139 step has not yet executed in any CI run — blocked upstream by `scripts/calibration-gate.sh` failing on a not-evidence-floor-aware `maxHardNegative` check (09-15→09-17) and the Swift-test semaphore-bound failure (09-18). **Upstream fix landed:** commit `c2eb7fa` made the calibration script evidence-floor-aware (gates `maxHardNegativeWithEvidence` at 0.99; raw max is info-only) — verified end-to-end green locally 2026-09-18 (F-3 regeneration 63.5s + validation pass; S2: the old rule reproduces the exact CI failure on the current artifact, the new rule passes). The RG-139 step remains receipt-pending until a CI run that includes `c2eb7fa` executes it. Enforcement is claimed only from the first run that shows the step `success`; ledger and falsifiers in `docs/audits/rg139-ci-wiring-and-runner-portable-corpus-paths-2026-09-18.md`. | `PASS` | `Tests/PDFEditorCoreTests/RasterBlendCalibrationGateTests.swift` (4 tests); extraction in `ContentInvariantRasterExtractor.swift` (gradedScales, edgeScales) + `LayoutFingerprintV2.swift` (raster-only-page gate, rotation-aware geometry); receipts in `docs/audits/blend-sweep-binding-constraint-analysis-2026-09-03.md` §follow-up and `docs/audits/graded-occupancy-implementation-2026-09-03.md` §addendum |
...
| Cross-provider OCR quality | `PASS` — RG-136: Tesseract avg WER 0.002, Vision 0.000, PaddleOCR 0.018 (post-reading-order, absolute 0.10) vs baseline (tolerance ±0.05) |
...
- **Scope:** CI runs the cross-provider OCR benchmark
  ...
  threshold (0.10 for Tesseract 5.5.0, Apple Vision, and — since the
  2026-09-11 re-baseline — PaddleOCR PP-OCRv6, derived from measured
  0.0–0.019, 0.0, and 0.0179 respectively — see
  `docs/audits/ocr-cross-provider-benchmark-2026-09-01.md`).
- **Baseline (re-generated 2026-09-11 with reading-order post-processing in
  the PaddleOCR lane, 8 fixtures, 4 providers, all rows OK):** Tesseract
  5.5.0 avg WER 0.0024; Apple Vision avg WER 0.000; PaddleOCR PP-OCRv6 avg
  WER **0.0179** (was 0.1091 at the 2026-09-03 baseline — the multi-column
  outlier 0.7297 collapsed to 0.0000 once the lane reordered boxes into
  bands/columns; the residual is a mixed-punctuation recognition miss,
  0.1429, pre-dating the fix); Marker (Surya) avg WER 0.0157 (reproduced
  exactly). Schema `pdf-editor.ocr-wer-baseline` v1.0 at
...
- **Regression-only gating for Marker (Surya); PaddleOCR re-classified
  2026-09-11:** Marker is in `GATE_REGRESSION_ONLY_PROVIDERS` — gated on
  regression-vs-baseline only, with no absolute corpus-average threshold,
  because its residual WER (multi-column 0.0541, mixed-punctuation 0.0714)
  is markdown-conversion design, not a defect. PaddleOCR was removed from
  that list when the reading-order fix made its corpus average
  engine-quality-dominated: it now carries the 0.10 absolute threshold
  (~5.6× headroom; a historical-mode ordering regression costs ~+0.125
  average and fails both threshold and baseline). A regression or engine
  error still fails the gate for either provider.
