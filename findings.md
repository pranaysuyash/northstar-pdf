...
| F-078 | Pipeline progressive-render test flakes under full-suite load | Verified by full-run failure + isolation pass; flake not regression | Full run recorded an issue at `PipelineRendererTests.swift:72` after 136.269s (5s safety timeout resumed nil); isolation re-run passed 8/8 in 2.135s with the progressive test at 0.547s; no product-code change between runs. | Closes the last residual full-suite issue as a load flake; optional hardening is load-aware timeout / pre-load sequencing, not a RenderingPipeline fix. |
...
### F-078: Pipeline progressive-render test flakes under full-suite load (nil completion within 5s safety timeout)

- **Date:** 2026-09-24
- **Status:** Verified by dual evidence — full-suite failure then isolation pass; classified as load-induced flake, not a product regression.
- **Observed Failure:** In the full background run (`/tmp/pdf-editor-swift-test-last.log`), `Pipeline progressive render produces valid image at each stage` recorded an issue at `Tests/PDFEditorCoreTests/PipelineRendererTests.swift:72:19` after **136.269s**. L72 is the `Issue.record("Progressive render did not complete")` inside the async `progressiveImageValid` helper, reached when the 5s safety-timeout `Task` resumes `nil` before a non-nil `RenderedPage` arrives from `RenderingPipeline.renderPageProgressive` (`Sources/PDFEditorCore/RenderingPipeline.swift:218`; early `completion(nil)` if `documentData` is nil; `loadDocument` at L185). The safety-timeout Task is only scheduled after `renderPageProgressive` returns, so either `loadDocument` or the progressive callback path stalled under parallel heavy-suite load (LayoutFingerprint ~198s, RasterBlend ~208s, AcroForm Parity ~156s concurrent) longer than the safety window plus the remainder of the 136s wall clock.
- **Isolation Proof:** `/tmp/pdf-editor-pipeline-rerun.log` (pid 32739): suite `Pipeline Renderer — 1st Principles` — **8 tests / 1 suite passed in 2.135s**; the progressive test alone passed in **0.547s**. Zero warnings in the re-run log. No product code change between runs.
- **Classification:** Load-induced test flake (resource contention on document load / main-actor progressive delivery under concurrent heavy suites), **not** a regression in `RenderingPipeline`. The full-suite wall was 1677.992s with 2 issues total; both issues closed by isolated re-runs (OCR semaphore + this one).
- **Hardening Paths (optional, not blocking):** (a) raise the 5s safety timeout to a load-aware bound analogous to `PDF_EDITOR_HEAVY_SEMAPHORE_ACQUIRE_BOUND`; (b) force `loadDocument` to complete before installing the safety timeout; (c) mark the test as a heavy-lane participant so it does not contend with the 1658-test parallel swarm.
- **Sources:** `/tmp/pdf-editor-swift-test-last.log`, `/tmp/pdf-editor-pipeline-rerun.log`, `Tests/PDFEditorCoreTests/PipelineRendererTests.swift` (L72), `Sources/PDFEditorCore/RenderingPipeline.swift` (L185, L218).
