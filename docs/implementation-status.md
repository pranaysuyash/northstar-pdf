...
- **D-010 Intent-Driven Editor Modes (`Read` / `Fill` / `Sign` / `Edit`):**
  ...
  - **Bounded Native Image Overlay (signature placement)**: `overlayImage` is serializable in the PDFKit lane since 2026-09-21 for **annotation-free, unrotated pages** — the signature image is baked into page content through the verified `PDFPage` drawing-override persistence path, with reopen-verified raster presence evidence at export (D-048 amendment; probe record: `docs/audits/pdfkit-overlay-image-limit-verification-2026-09-21.md`). Pages carrying annotations or rotation, named stamps, and AcroForm documents remain fail-closed with per-kind accurate reasons pending the form-aware writer (NM-T47).
  ...
  - **Form Flattening & Clean Export Pipeline**: `exportFlattenedCopy(destination:)` is currently fail-closed in the native PDFKit lane because the provider cannot prove that interactive fields, visual overlays, annotations, and signature stamps are baked into page content without losing semantics; a form-aware writer remains required.
  ...
  - **Authored Metadata Sanitization**: `sanitizeAndExportCopy(destination:)` publishes a separate, validated copy with authored title, author, subject, and creator fields scrubbed while preserving the source. PDFKit serializer provenance (`Producer`, `CreationDate`, `ModDate`) and deeper EXIF/XMP/attachment/action removal remain provider-specific gates and are not claimed by this native writer.
