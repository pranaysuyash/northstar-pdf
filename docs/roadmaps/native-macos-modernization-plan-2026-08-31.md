...
| 2026-09-01 | Bookmark-backed recent/drop identity increment | `AppModel` persists bounded bookmark records for successful opens; home drop requests an in-place file representation and falls back to an app-owned temporary copy when the provider cannot grant one; provider failures now surface an alert | T2 history/bookmark/moved-source fallback checks pass; revoked/reselected source cases and packaged drop proof remain open |
