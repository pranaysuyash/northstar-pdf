...
| `bash benchmark/test_independent_viewer.sh` | Pass for reopenability | Poppler `pdfinfo`/`pdftotext` and MuPDF `mutool info` independently reopened 35 eligible source and derived corpus PDFs, including encrypted native and browser parity outputs with their password; this does not clear qpdf or fidelity failures |
