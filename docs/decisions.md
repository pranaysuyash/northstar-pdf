...
## D-079: IncrementalFormWriter reads object streams (xref-stream documents leave the fail-closed rejection set)

- **Date:** 2026-09-08
- **Status:** Implemented, evidence complete; supersedes the "IncrementalWriter refuses compressed documents" clause of D-076's context
- **Context:** `compressed-acroform.pdf` (ObjStm-bearing, xref-stream base) was the last IncrementalWriter experimental fixture — the walker rejected it fail-closed (`objectNotFound`/compressed set thrown). Cross-reference stream and ObjStm reading is implemented: `XrefInfo.objectStreams` (type-2 entries: object → stream+index, merged newest-wins across the `/Prev` chain), `objectSpan` resolves type-2 objects by inflating the ObjStm and slicing the header-indexed body (trailing-EOL stripping in `inflateZlib` per PDF §7.3.8.1 — the raw decoder corrupted output without it, Observed: zero entries parsed on `noop.pdf`), and field-tree edits on compressed bases are appended as classic-object incremental updates (prefix preserved, qpdf `--check` clean, PDFKit round-trip re-reads the written `/V`). Measured limitation (canary-pinned, producer-independent): PDFKit's *reader* rejects any incremental update whose base is xref-stream/ObjStm-bearing — 3 hand-built qpdf-valid shapes all rejected, and PDFKit's own save silently normalizes the file to a 37KB full rewrite with zero ObjStm. The parity experiment classifies this as `hybrid-reopen-limitation` (excluded from radio round-trip scoring with the measured reason), not a writer defect. Also closed: `noop.pdf`'s walker failure was an `inflateZlib` bug, not a treeless fixture — qpdf shows a real AcroForm tree (falsified the "treeless fixtures out of domain" hypothesis).
- **Decision:** Object-stream documents are first-class for the incremental writer. The fail-closed contract narrows to genuinely unresolvable object references only. PDFKit-reopen failures on xref-stream bases are a documented Apple reader limitation (canary test), not an IncrementalWriter refusal.
- **Trade-offs:** The writer now accepts documents whose appended update PDFKit cannot open (qpdf-valid); callers that require PDFKit compatibility on such bases must do a full rewrite — this is surfaced via the documented limitation, not hidden.
...
## D-083: The Northstar shell is an owner-directed AI-native, agentic review-first workspace (resolves MAD-D1; amends D-066)

- **Date:** 2026-09-17
- **Status:** Accepted working decision (owner-directed)
- **Approval source:** owner directive 2026-09-17: "i have been asking for ai native, new age not some 20 yr old pdf reader thing… think, explore, convene council or multiple councils, document everything." This is the dated directive the adversarial seat identified as missing: the spatial critique in `docs/audits/modern_workspace_vision.md` §1 (visual cramping) and the agentic thesis (D-052 §5, X7) are hereby united as one direction.
- **Context:** MAD-D1 (document-first vs library-first vs AI-native Agent Desk) blocked MAD-I6 and the empty-state A1/R1 pick. Council 2026-09-17 (`docs/audits/shell-decision-council-2026-09-17.md`; seats PER-0172, PER-0922-standard, PER-0315, PER-91013 adversarial; Lead PER-0926) returned Conditional Go with five conditions, reconciled below.
- **Decision:**
  1. **Shell thesis:** the primary interaction surface is the D-078 agent loop (deterministic planner first; model assistance only behind `LocalAssistLane`), rendered in the vision doc's spatial language (pillars 1–4). Empty-state pick resolved: **A1 "Agent Desk" over R1 library.**
  2. **D-066 amended:** the archetype sentence "bounded document-based completion editor" is superseded as thesis statement and retained as the operating-constraint set ("agent workspace over a bounded document completion core"). Invariants unchanged: export-only lifecycle; bounded typed operations with a single mutation owner through AppModel/provider paths; scene-owned `DocumentSession` with digest-bound plans; D-078's six gate invariants; D-067 claim discipline; D-069 severity contract; the zero-egress doctrine; D-068 view/export discipline.
  3. **Document topology unchanged:** multiple independent windows stay. Multi-document workspace machinery remains declined behind NM-T39, re-scoped to own the surviving "whether" questions (library/corpus demand; agentic-organizer demand) via cohort instruments.
  4. **Claim discipline (layered):** "AI-native" is the direction (owner thesis language, decision records). User-facing claims use D-052 §5 vocabulary ("review-first agents whose output is a validated document mutation with provenance"). The "Agent Desk" product name may appear in UI only after the spine slice's oracle passes; "AI-native" stays out of UI/marketing until a second model lane exists beyond `LocalAssistLane`. The $4.99 Agent+ cloud lane remains absent and stays its own D-gate (D-052 P95 economics re-run first).
  5. **Slice sequence** (each slice enters task-inventory with exit oracle + evidence tier + named falsifier at creation time):
     - **Slice 0 (immediate, no new UI; NM-T41):** X7 unification design doc — vocabulary, authority boundaries, execution-route table per capability (on-device Foundation Models / deterministic / absent / deferred-cloud); run-journal persistence verified (persist `AgentRunJournal` if in-memory); PL-D09 cohort instrument with pre-registered kill thresholds: <30% of cohort sessions open the agent loop, or >50% of approved plans abandoned ⇒ the organizing-surface thesis is falsified and the loop remains a wedge tool.
     - **Slice 1:** "Teach Northstar this workflow" through the wired D-078 loop (recurring-form flow: drop → recurrence → plan sheet → per-step approval → bulk fill → receipt → save as workflow), with the inspectable journal/plan surface. Oracle: every rendered entry traces to a journal/receipt row (anti-fabrication). Outcome test: a user states one goal and in under two minutes holds a validated, receipted mutation or a saved reusable workflow.
     - **Slice 2:** goal capture as the shell's organizing input; open-ended goals degrade honestly to ranked command search.
     - **Slice 3:** escalation/retry honesty surface (reason codes; visible "ledger unchanged" terminal state).
     - **Slice 4:** AI-native empty state; CTAs only to planner-supported goals; "Agent Desk" name ships here at the earliest.
     - **Parallel styling track (unblocked now, never the headline):** vision phases 2–5 as styling on existing surfaces; each surface passes MAD-R1/R2 accessibility + human-visual gate evidence before shipping.
  6. **Sequencing:** the D-055 launch critical path remains sole P0 ordering owner; shell slices are paraxial extensions on the wired loop, not interrupts. The T01 ownership matrix must be accepted and the AppModel seams the shell touches extracted (A-9/X6) before the first new shell surface; the shell provides the multi-concern pressure evidence X6 required.
  7. **Vision doc typed:** `docs/audits/modern_workspace_vision.md` is an aspiration blueprint (T0), owner-directed — not status authority (D-055) and not evidence of shipped state; its §4 fidelity claim is restated in observation form.
- **Falsifiers (per slice, never on the direction):** slice-0 kill thresholds (above); X7's ledger-reuse usability falsifier (fall back to a parallel side panel with explicit apply gates); any slice-4 CTA reaching an unimplemented path (D-071 orphan class); an instrument that lands but is never consumed (X2 rule); any shell slice that cannot ship without violating D-067 or D-069 is declined until it can.
- **Trade-offs:** direction is owner-asserted rather than evidence-gated (D-078 precedent); the compensating discipline is per-slice falsifiers plus instruments scoped to depth/adoption/next-slice — never retrospective direction validation. The visual track proceeds unconditionally; the agentic track carries the kill-test.
