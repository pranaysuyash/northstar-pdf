...
| RG-081 | Release runbook | `PARTIAL` | Reproducible ordered runbook written 2026-08-25 (`docs/release-runbook.md`) and extended same day: veraPDF PDF/UA validation (`tools/verapdf --flavour ua1 ...`, RG-004), corpus-sweep regeneration (`Tests/fixtures/generate_corpus_sweep.py` + `Tests/fixture-corpus-sweep_test.mjs`), and the continuous-view perf measurement (`Tests/perf-continuous-view_test.mjs`). **CI wiring delivered 2026-08-26** (`.github/workflows/ci.yml`): four independent gates (Swift test + release build / 31 pure-Node contract tests / Playwright browser E2E / tool-dependent tests with graceful skip). `Tests/run-tool-dependent-tests.mjs` detects qpdf, poppler, pikepdf, verapdf at runtime and skips unmet dependencies with clear diagnostics. CI installs qpdf+poppler via brew and pikepdf via pip; skipped tests are advisory, not failures. `Tests/web_editor_workflow_test.mjs` self-boots its static server — zero external choreography. Local pre-push hook mirror at `tools/pre-push-hook.sh`. Remaining: full 78-file E2E suite stabilization, CI runner tool-version pinning. Verification at audit time: 244/244 Swift tests, 51 web contract checks, workflow test self-contained, tool-dependent gate 2/2 pass (13 skipped for missing pikepdf locally). |
...
| RG-088 | Independent review | `PARTIAL` | **First-pass review delivered 2026-08-26** (`docs/audits/independent-adversarial-review-per-0206-2026-08-26.md`) under PER-0206 (Post-Fix Adversarial Reviewer) as a distinct analytical lens. Found 5 observations (1 MEDIUM, 4 LOW); no overclaiming detected. Structural independence (separate implementation capability) remains the gap for full PASS. |
