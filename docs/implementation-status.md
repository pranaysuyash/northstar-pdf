...
| Native/web serialized contract parity harness | Pass baseline, Tier 2/S1 native plus Tier 3/S1 browser; first mismatch record preserved | `node Tests/pdf_contract_parity_test.mjs` emitted native and browser bundles for all ten manifest entries, matched source digests and malformed-input failure behavior, and recorded 75 normalized semantic mismatches in `benchmark/results/contract-parity-2026-08-24/parity-report.json` |
...
| Independent viewer gate | Pass, Tier 2/S1 | `bash benchmark/test_independent_viewer.sh` independently reopened 35 eligible corpus PDFs through Poppler `pdfinfo`/`pdftotext` and MuPDF `mutool info`, including encrypted native and browser parity outputs; this does not clear structural or fidelity failures |
