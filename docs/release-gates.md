...
| RG-006 | Native VoiceOver workflow | Native/accessibility | `PARTIAL` | VoiceOver observation covers all reader controls, errors, and navigation |
| RG-007 | Browser screen-reader workflow | Web/accessibility | `PARTIAL` | Screen-reader observation covers landmarks, text layer, search, status, and password flow |
...
| RG-014 | Signed-document behavior | Native/web/security | `PARTIAL` | Detection + invalidation guard and `validateSignatureIntegrity` delivered 2026-08-25: detects `/SigFields`, `/SigFlags`, `/ByteRange`+`/Contents`, validates byte-range structure, and optionally verifies detached CMS with OpenSSL while leaving certificate trust unevaluated. Edit sessions block until explicit acknowledgment; unsigned and synthetic signed fixtures pass expected state tests. Real trusted signed corpus, revocation, long-term validation, and preservation remain open |
| RG-015 | XFA behavior | Native/web/provider | `PARTIAL` | Detect + safe-reject delivered 2026-08-25 (`web/pdf-xfa-guard.mjs`): detects `/XFA` (stream vs packet-array kind), extracts packet names and a dynamic hint (config/dynamicRender packets); `assertNoXfaFormEdits` refuses native-field edit operations with an explicit no-false-AcroForm-claim reason while leaving overlayText untouched; malformed facts rejected (falsifier). Validated on the real AcroForm fixture (no false claim) and a synthetic stream-XFA fixture. Full static/dynamic/hybrid taxonomy and real XFA corpus remain open |
...
| RG-043 | Search status announcements | Native/web/accessibility | `PARTIAL` | Match counts, current result, page changes, and no-match state are announced |
...
| RG-057 | Focus restoration | Native/web/accessibility | `PARTIAL` | Focus returns predictably after modal, search, link, error, and jump actions |
| RG-058 | Reduced motion | Native/web/accessibility | `PARTIAL` | Motion preferences are respected across reader transitions |
